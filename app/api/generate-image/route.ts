...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Always append no-text instruction to every prompt
    const noTextSuffix = ", photorealistic, no text, no words, no letters, no writing, no captions, no signs, no labels"
    
    const finalPrompt = isTeen
      ? `${prompt}, modern anime style, vibrant colors, expressive characters, clean linework, contemporary anime aesthetic${noTextSuffix}`
      : `${prompt}${noTextSuffix}`

    // Strong negative prompt to prevent ANY text from appearing
    const negativePrompt = "text, words, letters, writing, captions, labels, watermark, signature, typography, font, alphabet, numbers, digits, symbols, logos, banners, signs, subtitles, title, headline, caption, inscription, characters, glyphs, script, handwriting, printed text, book, scroll, parchment, paper with writing"
